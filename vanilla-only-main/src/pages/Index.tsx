...
const StudyHub = () => {
  ...
  const renderMap = () => {
    const centerX = 400;
    const centerY = 350;
    const radiusSubject = 180; // spread out more
    const radiusTopic = 100;   // keep topics at a good distance

    const subjectsArray = Object.keys(conceptMapData);
    ...
    return (
      <div className="max-w-6xl mx-auto text-center">
        ...
        <div className="study-card overflow-x-auto">
          <svg width="900" height="750" className="mx-auto">
            {/* Center Hub */}
            <circle cx={centerX} cy={centerY} r="60" fill="#1d3557" />
            <text
              x={centerX}
              y={centerY + 5}
              textAnchor="middle"
            ...
            {subjectsArray.map((subject, i) => {
              const angle = (i * 2 * Math.PI) / subjectsArray.length;
              const sx = centerX + Math.cos(angle) * radiusSubject;
              const sy = centerY + Math.sin(angle) * radiusSubject;
              ...
              return (
                <g key={subject}>
                  <line
                    ...
                    strokeWidth="1.5"
                  />
                  <circle cx={sx} cy={sy} r="45" fill={color} opacity="0.9" />
                  <text
                    x={sx}
                    y={sy + 5}
                    textAnchor="middle"
                  ...
                  {conceptMapData[subject].map((topic, j) => {
                    const topicAngle =
                      angle + (j - conceptMapData[subject].length / 2) * (Math.PI / 10);
                    const tx = sx + Math.cos(topicAngle) * radiusTopic;
                    const ty = sy + Math.sin(topicAngle) * radiusTopic;
                    ...
                    return (
                      <g key={topic}>
                        <line
                          ...
                          strokeWidth="1"
                        />
                        <circle cx={tx} cy={ty} r="30" fill="#f1faee" />
                        <text
                          x={tx}
                          y={ty + 4}
                          textAnchor="middle"
  ...
  const conceptMapData = {
    Java: ["OOP", "Collections", "Multithreading", "JDBC", "Streams", "Generics", "Spring Basics"],
    DAPY: ["NumPy", "Pandas", "Matplotlib", "EDA", "Data Cleaning", "Visualization", "SciPy"],
    DCN: ["OSI Layers", "TCP/IP", "Routing", "Protocols", "Switching", "DNS", "Congestion Control"],
    DSA: ["Arrays", "Linked Lists", "Stacks/Queues", "Trees", "Graphs", "Heaps", "Dynamic Programming"],
    AI: ["Search", "ML Basics", "Neural Nets", "NLP", "Computer Vision", "Reinforcement Learning", "Ethics"],
    WT: ["HTML", "CSS", "JavaScript", "Responsive Design", "APIs", "DOM", "Web Security"]
  };
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, i) => (
                    <div
                      key={i}
                      className={`w-2 h-2 rounded-full ${i < topic.difficulty ? 'bg-cyan-400' : 'bg-gray-600'
                        }`}
                    />
                  ))}
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          ...
          <nav className="flex gap-6 mt-4">
            {[
              ...
              <button
                key={item.key}
                className={`px-4 py-2 rounded-lg transition-colors ${currentView === item.key
